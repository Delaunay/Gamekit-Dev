...
#define GENPRINT(fmt, ...) UE_LOG(LogBCGen, Display, TEXT(fmt), __VA_ARGS__);
...
void KismetCodeTransform::CallFunction(UK2Node_CallFunction *Node)
{
    ...
    if (Outs.Num() > 0)
    {
        GENPRINT("    %s = %s(%s)", *Join(",", Outs), *Node->GetFunctionName().ToString(), *Join(",", Args));
    }
    else
    {
        GENPRINT("    %s(%s)", *Node->GetFunctionName().ToString(), *Join(",", Args));
    }
...
void KismetCodeTransform::DynamicCast(UK2Node_DynamicCast *Node)
{
    ...
    GENPRINT("    %s = Cast(%s, %s)", *Join(",", Outs), *Node->TargetType->GetName(), *Join(",", Args));
